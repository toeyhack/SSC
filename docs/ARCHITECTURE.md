...
Migration `0009_wave1_ssc_evaluators` adds an immutable catalog-issue-version foreign key to each rule version. The reviewed Wave 1 and Wave 2 registries activate only against issue definitions belonging to an attested real `SSC_API` snapshot. Taxonomy changes therefore create/reconcile a new evaluator version after approval, while historical findings retain both exact rule and exact issue definitions.
...
| Executors | `app.services.scan_executors` | Bounded port/path lists, request/read limits, socket timeouts and same-target/configured-port redirects; preserve multi-value headers, normalized redirect chains and certificate-chain metadata |
...
| Rule evaluation | `app.services.rule_evaluation`, `app.services.wave1_rules`, `app.services.wave2_rules`, `app.services.wave3a_rules` | Deterministic tri-state observation results and exact version-pinned Wave 1/2/3A rule definitions; no scoring inference |
| Service probes | `app.services.service_probes` through the TCP executor | One bounded adapter interface with isolated protocol parsers; no port-only inference, authentication, enumeration, or state-changing commands |
...
HTTP probes use configured HTTP/HTTPS ports and explicit same-target paths, bounded response reads, multi-value normalized security headers, CSP meta policies, redacted cookie attributes and constrained redirects. TLS records certificate evidence and bounded version-pinned handshakes for TLS 1.0 through 1.3; constrained cipher groups preserve the offered policy catalog and actual accepted suite. Unsupported or inconclusive probes are `null`, not evidence of safety. DNS preserves exact TXT query names, response classes, normalized records, SPF/DMARC analyses, two unpredictable SPF-wildcard nonce names, and policy results only for explicitly declared subdomains. DNS-only targets do not need A/AAAA records. TCP performs connections only to explicit ports, with no automatic range scan. OS hostname resolution and aggregate scan duration are not a global deadline; further scheduling/concurrency/operational controls belong to hardening.
...
Catalog changes create `catalog_issue_type_versions`; rule changes create `rule_engine_rule_versions`. SSC-aligned rule versions additionally pin `catalog_issue_type_version_id`, and findings copy that exact link. Each new scan summary captures rule assessment statuses, factor identity and affected target identity, so current metadata changes cannot alter normalization of a completed run.
